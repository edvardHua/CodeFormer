...
class FFHQBlindJointDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        # 输入和输出gt，有时候输入并不一定是由gt退化而来
        self.input_folder = opt.get("dataroot_input", None)
        ...
        if self.io_backend_opt["type"] == "lmdb":
            ...
            if self.input_folder is not None:
                self.input_paths = paths_from_folder(self.input_folder)
    ...
    def __getitem__(self, index):
        ...
        if self.input_folder is not None:
            ...
            img_bytes = self.file_client.get(input_path)
            img_in = imfrombytes(img_bytes, float32=True)

        if self.input_folder is not None:
            img_res, status = augment(
                [img_in, img_gt],
                hflip=self.opt["use_hflip"],
                rotation=False,
                return_status=True,
            )
            img_in, img_gt = img_res
        else:
